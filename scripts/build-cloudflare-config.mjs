...
const redirects=['/index.html / 301','/animales/:slug/index.html /animales/:slug/ 301',...verified].join('\n')+'\n';
fs.writeFileSync(path.join(root,'_headers'),headers,'utf8');
fs.writeFileSync(path.join(root,'_redirects'),redirects,'utf8');
console.log(`Configuración Cloudflare generada (${verified.length} redirecciones históricas verificadas).`);
